...
const Cart = () => {
  ...
  // ✅ تعريف snackbarConfig مرة واحدة بدل ما تتكرر
  ...
  // ✅ حذف التعريف المكرر — بيجي من useSnackbar بس
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {cart.length > 0 ? (
          ...
          >
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {/* ✅ إصلاح onClick — كان بيبعت الـ event مش بيفتح المودال */}
              <button
                onClick={() => setProductsToClear(true)}
                className="
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            {cart.length > 0 ? (
              <AnimatePresence>
                {filteredCart.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 mt-5 px-3">
                    {filteredCart?.map((product, index) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          <span className="text-2xl font-semibold text-[#5c3d1e] dark:text-[#bd9e7d]">
                            ${product.price}
                          </span>
          ...
          {cart.length > 0 && (
            ...
            >
              <div
                className="rounded-3xl p-6 sticky top-6 bg-white dark:bg-black border border-[#e8dfd0] shadow-[0_4px_24px_rgba(100,70,40,0.08)] px-3"
              >
                {/* Sidebar Header */}
                <div
                ...
                >
                  ...
                  >
                    {cart.reduce((totalQty, product) => totalQty + product.quantity, 0)}
                  </span>
                </div>

                {/* Subtotal */}
                <div className="flex justify-between items-center mb-3">
                  <span className="text-sm font-light text-[#776a5d] dark:text-gray-300">{t("subtotal")}</span>
                  <span className="text-sm font-medium text-[#3d2b1a] dark:text-gray-300">${total.toFixed(2)}</span>
                </div>

                {/* Shipping */}
                <div className="flex justify-between items-center mb-3">
                  <span className="text-sm font-light text-[#776a5d] dark:text-gray-300">{t("shipping")}</span>
                ...
                <div
                  ...
                  style={{ borderBottom: "1px solid #f0e8da" }}
                >
                  <span className="text-sm font-light text-[#776a5d] dark:text-gray-300">{t("tax")} (10%)</span>
                  <span className="text-sm font-medium text-[#3d2b1a] dark:text-gray-300">${(total * 0.1).toFixed(2)}</span>
                </div>

                {/* Total */}
                <div className="flex justify-between items-center mb-6">
                  <span className="text-base font-medium text-[#1a1410] dark:text-gray-400">{t("total")}</span>
                  <span
                ...
                <button
                  ...
                  style={{ border: "1px solid #c8b49a", background: "transparent" }}
                ...
                <div className="flex justify-center gap-3">
                  ...
                  <motion.button
                    ...
                    onClick={() => {
                      setLoading(true);
                      setTimeout(() => {
                        const deletedProduct = cart.find((p) => p.id === productToDelete);
                        dispatch(deleteFromCart(productToDelete));
                        setProductToDelete(null);
                        setLoading(false);
                        // ✅ استخدام مفتاح ترجمة صح مع {{product}} بدل template literal
                        enqueueSnackbar(
                          t("deletedFromCart", { product: deletedProduct?.title }),
                          snackbarConfig
                        );
                      }, 500);
                    }}
                  ...
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      setTimeout(() => {
                        ...
                        enqueueSnackbar(t("cartCleared"), snackbarConfig);
                      }, 600);
                    }}
                    className="cursor-pointer px-5 py-2.5 rounded-full text-sm font-light text-white transition-all duration-200"
                    style={{ background: "#5c3d1e" }}
