...
const Products = ({ limit, showFilter = true, showBread = true, showPagination = true,showHeroSrip=true ,showBottomBanner=true,showNumberOfProducts=true}) => {
  const [currentPage, setCurrentPage] = useState(1);
  const productsPerPage = 8;
  const navigate = useNavigate();
  const [filters, setFilters] = useState({ category: "", brand: "", price: "", sort: "" });
  ...
  if (filters.category) filteredProducts = filteredProducts.filter(p => p.category?.toLowerCase() === filters.category.toLowerCase());
  if (filters.brand) filteredProducts = filteredProducts.filter(p => p.brand?.toLowerCase() === filters.brand.toLowerCase());
  if (filters.price === "low") filteredProducts.sort((a, b) => a.price - b.price);
  if (filters.price === "high") filteredProducts.sort((a, b) => b.price - a.price);
  if (filters.sort === "newest") filteredProducts.sort((a, b) => b.id - a.id);
  if (filters.sort === "rating") filteredProducts.sort((a, b) => b.rating - a.rating);
  if (filters.sort === "discount") filteredProducts.sort((a, b) => b.discountPercentage - a.discountPercentage);
  ...
  return (
    <div className=" min-h-screen">
     ...
     {showHeroSrip&& <div
        ...
        style={{ background: "linear-gradient(135deg, #1a1410 0%, #3d2b1f 50%, #6b4c36 100%)" }}
      >
        {/* Decorative circles */}
        <div style={{ position: "absolute", top: -60, right: -60, width: 280, height: 280, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.06)" }} />
        <div style={{ position: "absolute", top: -30, right: -30, width: 180, height: 180, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.06)" }} />
        <div style={{ position: "absolute", bottom: -80, left: -40, width: 300, height: 300, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.04)" }} />
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs uppercase tracking-[0.35em] text-amber-400 font-medium">ShopEasy Store</span>
              </div>
              <h1
            ...
            <div className="flex gap-6 md:gap-10">
              {[
                { n: products.length, label: "Total Items" },
                { n: categories.length, label: "Categories" },
                { n: brands.length, label: "Brands" },
              ].map((s, i) => (
                <div key={i} className="text-center">
                  <p className="text-2xl font-light text-white" style={{ fontFamily: "'Georgia', serif" }}>{s.n}</p>
      ...
      {showFilter && (
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-[#ede8e0] ">
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
            ...
            {hasActiveFilters && (
              <button
                onClick={() => setFilters({ category: "", brand: "", price: "", sort: "" })}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        {/* Result count */}
     { showNumberOfProducts&&  <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-gray-500">
            Showing <span className="text-[#1a1a1a] font-medium">{startIndex + 1}–{Math.min(startIndex + productsPerPage, filteredProducts.length)}</span> of <span className="text-[#1a1a1a] font-medium">{filteredProducts.length}</span> results
          </p>
          {hasActiveFilters && (
            <div className="flex flex-wrap gap-2">
              {Object.entries(filters).filter(([, v]) => v).map(([k, v]) => (
                <span key={k} className="text-[11px] bg-amber-50 text-amber-700 border border-amber-200 px-3 py-1 rounded-full capitalize">
                  {k}: {v}
                </span>
              ))}
            </div>
        ...
        <AnimatePresence mode="wait">
          {displayedProducts.length === 0 ? (
            ...
            >
              ...
              <p className="text-xl font-light text-gray-400" style={{ fontFamily: "'Georgia', serif" }}>No products found</p>
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden rounded-xl mb-4">
                      <img
                        ...
                        alt={product.title}
                        className="border border-gray-50 border-2  w-full rounded-2xl h-48 object-contain transition-transform duration-500 group-hover:scale-110"
                      />
                      <div
                        onClick={(e) => { e.stopPropagation(); if (!isInCart) dispatch(addToCart(product)); }}
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-xl font-semibold text-[#776a5d]">${product.price}</span>
          ...
          <div className="mt-16 flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {getPagination().map((page, index) =>
                page === "..." ? (
                  <span key={index} className="w-8 text-center text-gray-400 text-sm">···</span>
                ) : (
                  <button
                    key={index}
      ...
      {/* ══ BOTTOM BANNER ══ */}
   { showBottomBanner &&  <div className="max-w-7xl mx-auto px-4 py-12">
        <div
          className="rounded-3xl px-8 md:px-16 py-12 flex flex-col md:flex-row items-center justify-between gap-8"
          style={{ background: "linear-gradient(135deg, #1a1410 0%, #3d2b1f 60%, #6b4c36 100%)" }}
        >
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-amber-400 mb-3">Limited time</p>
            <h3
            ...
            <p className="text-gray-400 text-sm mt-3 font-light">Use code <span className="text-amber-300 font-medium tracking-wider">WELCOME20</span> at checkout</p>
          </div>
