      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            >
              {t("All")} <span className="italic text-amber-200">{t("Products")}</span>
            </h1>
            <p className="text-gray-400 text-sm mt-3 font-light">
              {filteredProducts.length} {t("products")}
              {hasActiveFilters && (
                <span className="text-amber-400 ml-1">· {t("filtered")}</span>
              )}
            </p>
          </div>
          ...
          <div className="flex gap-6 md:gap-10">
            {[
              { n: data.length, label: t("TotalItems") },
              { n: categories.length, label: t("Categories") },
              { n: brands.length, label: t("Brands") },
            ].map((s, i) => (
      ...
      <div className="flex items-center justify-between mb-6">
        <p className="text-sm text-gray-500">
          ...
          <span className="text-[#1a1a1a] dark:text-gray-400 font-medium">
            {startIndex + 1}–{Math.min(startIndex + productsPerPage, filteredProducts.length)}
          </span>{" "}
          {t("of")}{" "}
          <span className="text-[#1a1a1a] dark:text-gray-400  font-medium">
      ...
      <div className="mt-16 flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {getPagination().map((page, index) =>
            page === "..." ? (
              <span key={index} className="w-8 text-center text-gray-400 text-sm">
                ···
              </span>
            ) : (
              <button
        ...
        <div className="flex items-center gap-2 mt-1">
          <span className="text-xs text-gray-400 ">{t("goToPage")}</span>
          <input
            type="number"
            min={1}
            max={totalPages}
      ...
      >
        <div>
          ...
          <h3
            ...
            style={{ fontFamily: "'Georgia', serif" }}
          >
            {t("get")} <span className="italic text-amber-200">{t("20%off")}</span>
            <br />
            {t("yourfirstorder")}
          </h3>
          <p className="text-gray-400 text-sm mt-3 font-light">
            {t("useCode")} <span className="text-amber-300 font-medium tracking-wider">SHOPEASY20</span> {t("atCheckout")}
          </p>
        </div>
